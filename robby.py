...
class Robby:
    ...
    def train(self, episodes=5000, steps=200, learning_rate=0.2, discount=0.9, epsilon=.1, graphics=False):
        ...
        for episode in range(episodes):
            ...
            for step in range(steps):
                ...
                # Update Q
                if not done:
                    self.q[self.state][action] = self.q[self.state][action] + learning_rate * (
                        reward + discount * np.amax(self.q[next_state]) - self.q[self.state][action])
                else:
                    self.q[self.state][action] = self.q[self.state][action] + \
                        learning_rate * (reward - self.q[self.state][action])
                ...
                # Finish episode if all cans collected
                if done:
                    break
            # Print out numbers
            if episode % 5 == 0:
    ...
    def test(self, episodes=5000, steps=200, epsilon=0.1, graphics=False):
        episode_rewards = 0
        reward_list = []
        for episode in range(episodes):
            # create new environment
